...
The prompt fix above was NOT sufficient. 4 of 8 workshops (`star_proof`, `role_depth`, `problem_solving`, `handling_uncertainty`) have a **fundamental design flaw**: they generate content from the resume + JD instead of helping users structure their own specific stories. For a behavioral question, the workshop can generate vague process descriptions about the target job instead of restructuring the user's actual incident. No amount of prompt engineering fixes this because **the specific incident lives in the user's head, not in any data passed to Haiku**. The resume lists responsibilities, not incidents. The original answer was vague (that's why it was flagged). Haiku can't restructure vagueness into specificity.
...
- **Handling Uncertainty Lesson** — first pass replaced the workshop with a short reset drill (Rephrase → Pause → Pick a Lane → Start). Then **expanded** into a full Duolingo-style 8-part interactive lesson (this commit, replaces the thin version).
  - **8 parts**: (1) Why rambling happens (flip cards + check), (2) The reset line (formula + phone-screen variant + flip cards), (3) Rephrase the question (tap-to-reveal examples + check), (4) Cue words (good/better example + 2 checks), (5) Pick the lane (6 tap-to-reveal lane cards + 2 lane-picker checks), (6) Start clean (weak-vs-strong reveal pairs + check), (7) Mid-answer reset (phrases + 2 recovery checks), (8) Final 5-question quiz from the difficult-question bank. **No "apply to your flagged question" step** (user explicitly removed it).
  - **Quiz format**: 5 questions, one of each format — rephrase / cue words / lane / stronger start / mid-answer reset. Pass at 4/5. Built via `buildQuiz()` which samples 4 distinct bank questions + 1 recovery scenario. Reshuffles on retry.
  - **Interactive primitives**: `FlipCard` (3D-feel tap-to-flip), `RevealList` (formal→plain hidden-then-shown), `LaneCard` (6 tap-to-reveal lane definitions), `WeakStrongPair` (weak shown / strong hidden then reveal with reasoning), `CheckQuestion` (MCQ with check/feedback/continue gating), `LaneCheck` (lane picker, randomized 4 of 7), `QuizScreen` (handles all 5 quiz formats), `CueChipRow` (good vs better chips), 8-segment top progress bar.
  - **Question bank** (`lib/handling-uncertainty-bank.ts`): 7 `DifficultQuestion`s seeded from the real HR-screen curveball pool (`lib/interview-prompts/hr_screen.ts:81-87`) + 1 added behavioral story Q. Each has `simplifiedRephrase`, `correctLane`, `weakStarts`, `strongStarts`, `distractorRephrases`, `cueWordOptions` (NEW — 4 cue-word sets per Q with one correct), `explanation`. Plus `LaneInfo` with `whenToUse` + `starter` per lane, 3 `RECOVERY_SCENARIOS`, reset phrase bank, and `buildQuiz()` generator.
