...
`
    ...
    systemPrompt += '\n        "clarity": <number 1-10 - how clearly did they express their thoughts?>,'
    systemPrompt += '\n        "articulation": <number 1-10 - how well did they articulate complex ideas?>,'
    systemPrompt += '\n        "pacing": <number 1-10 - appropriate speed, pauses, not rushing or dragging?>,'
    systemPrompt += '\n        "tone_appropriateness": <number 1-10 - professional, warm, appropriate for interview?>,'
    systemPrompt += '\n        "active_listening": <number 1-10 - responded to what was actually asked, built on interviewer cues?>,'
    systemPrompt += '\n        "professional_language": <number 1-10 - vocabulary, grammar, no filler words, no slang?>'
    systemPrompt += '\n      },'
    systemPrompt += '\n      "professionalism": {'
    systemPrompt += '\n        "appropriate_greeting": <boolean - did they greet professionally?>,'
    systemPrompt += '\n        "appropriate_closing": <boolean - did they close professionally or did interview end abruptly?>,'
    systemPrompt += '\n        "respectful_tone": <boolean - maintained respectful tone throughout?>,'
    systemPrompt += '\n        "prepared_environment": <boolean - no background noise issues, seemed prepared?>,'
    systemPrompt += '\n        "phone_etiquette": <boolean - didn\'t interrupt, appropriate turn-taking?>'
    systemPrompt += '\n      },'
    systemPrompt += '\n      "interest_and_enthusiasm": {'
    systemPrompt += '\n        "company_knowledge": "<none|limited|moderate|strong> - how much did they know about the company?>,'
    systemPrompt += '\n        "energy_level": "<low|moderate|high> - their overall energy and engagement level>,'
    systemPrompt += '\n        "tone_enthusiasm": "<flat|neutral|engaged|enthusiastic> - how enthusiastic did they sound?>,'
    systemPrompt += '\n        "follow_up_questions": "<none|generic|some_thoughtful|multiple_insightful> - quality of questions they asked the interviewer>"'
    systemPrompt += '\n      },'
    systemPrompt += '\n      "response_quality": {'
    systemPrompt += '\n        "questions_answered_directly": "<fraction like 1/2 - how many questions did they actually answer vs deflect?>,'
    systemPrompt += '\n        "questions_with_strong_examples": "<fraction like 0/2 - how many answers included specific examples with details?>,'
    systemPrompt += '\n        "questions_with_vague_answers": "<fraction like 1/2 - how many answers were vague or generic?>,'
    systemPrompt += '\n        "avg_response_length": "<description like ~66 seconds or ~45 words>"'
    systemPrompt += '\n      },'
    systemPrompt += '\n      "red_flags": {'
    systemPrompt += '\n        "flags_detected": [<array of objects with "flag_type", "severity" (low|medium|high), "description", "evidence" for each red flag found. Empty array if none.>]'
    systemPrompt += '\n      }'
    systemPrompt += '\n    }'
    systemPrompt += '\n  },'
    systemPrompt += '\n  "time_management_analysis": {'
    systemPrompt += '\n    "per_question_timing": [<array of timing objects with question_id, question_text, candidate_response_time, duration_seconds, assessment ("too_long"|"appropriate"|"too_short"), target_range>],'
    systemPrompt += '\n    "total_interview_duration": "<formatted duration like 2:25 - MUST be actual duration from transcript>",'
    systemPrompt += '\n    "target_duration": "<target duration like 15-20 minutes>",'
    systemPrompt += '\n    "duration_assessment": "<under_target|within_target|over_target> - honest assessment of whether duration met the target",'
    systemPrompt += '\n    "variance": "<variance from target, e.g., -12:35 if interview was way too short>",'
    systemPrompt += '\n    "questions_asked": <number of questions actually asked - count from transcript>,'
    systemPrompt += '\n    "questions_target": "<target range like 6-8>",'
    systemPrompt += '\n    "questions_assessment": "<below_target|within_target|above_target> - honest assessment of whether question count met the target",'
    ...
    systemPrompt += '\n- traditional_hr_criteria (with scores, feedback, AND sub_components objects containing ALL 6 required criteria listed above)'
    systemPrompt += '\n- time_management_analysis'
    systemPrompt += '\n- question_analysis'
    systemPrompt += '\n- next_steps_preparation'
    systemPrompt += '\n- comparative_analysis'
    ...
    systemPrompt += '\nYou MUST include ALL 6 criteria in "scores", "feedback", AND "sub_components" objects with these EXACT names:'
    ...
    systemPrompt += '\n5. response_quality'
    systemPrompt += '\n6. red_flags'
    systemPrompt += '\n\nDO NOT include culture_fit_indicators - that is assessed in a separate interview stage.'
    systemPrompt += '\nDO NOT use alternative names like "communication", "cultural_fit", "job_alignment", etc.'
    systemPrompt += '\nDO NOT omit any of these 6 criteria. Every single one must be present with a score, feedback text, AND sub_components.'
    systemPrompt += '\nThe sub_components MUST reflect your actual evaluation of each sub-dimension independently. DO NOT copy the parent score into all sub-components.'
    systemPrompt += '\nFor example, a candidate might have strong clarity (8/10) but weak active_listening (3/10) — evaluate each honestly.'
    systemPrompt += '\nIf you cannot assess a sub-component due to limited data, score it low and explain why in the feedback.'
