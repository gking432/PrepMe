...
const FRAMEWORK_GUIDANCE: Record<string, string> = {
  ...
  present_past_future: 'Evaluate whether the draft works as a professional introduction using Identity, Foundation, Recent Focus, and Direction. Be strict about generic language. A weak draft can have the right sections but still fail because each section is only a label with no qualifier. A strong draft needs a real professional identity, a real foundation, a recent focus, and a direction-and-fit explanation. Reject answers that say things like "I work in operations" or "I worked in different roles" or "I want a good next step" without a concrete qualifier that makes the section believable.',
  ...
  handling_uncertainty: 'Evaluate whether the draft handles uncertainty as Pause, Frame, Answer, Support. A strong draft should buy a second, narrow the lane, answer directly, support briefly, and stop. Reject hedge-heavy answers that circle the topic, sound more uncertain than honest, or never land a clear point.',
  career_alignment: 'Evaluate whether the draft makes the move sound logical and intentional. A strong draft should point to something specific about the role, connect that to relevant background, and explain why the timing makes sense now. Reject generic or selfish-first job-seeking language like "great opportunity," "ready for a new challenge," "growth," "title," or answers that could apply to almost any role.',
  pace_and_flow: 'Evaluate whether the draft can be practiced naturally as conversation rather than memorized as a script. A strong draft should have a cue-card headline, clear spoken beats, natural pause points, simple transitions, and a settled ending. Do not pretend to measure exact pause length from text alone. Instead, judge recognizable rhythm patterns like abrupt starts, stacked clauses, awkward turn-taking, or answers that read like a memorized dump instead of back-and-forth.',
}
