...
function normalizeStage(stage?: string) {
  if (stage === 'final_interview') return 'final'
  if (stage === 'team_interview') return 'culture_fit'
  if (stage === 'hiring_manager' || stage === 'culture_fit' || stage === 'final') return stage
  return 'hr_screen'
}
...
export default function PracticeForSessionPage() {
  ...
  useEffect(() => {
    ...
    if (sessionId === 'mock-session-preview') {
      const list = Array.isArray(MOCK_FEEDBACK.hr_screen_six_areas.what_needs_improve)
    ...
    async function load() {
      try {
        ...
        const normalizedStage = normalizeStage(sessionRow?.stage)
        if (sessionRow?.stage) setStage(normalizedStage)
        const ts =
        ...
        const sixAreaKey = `${normalizedStage}_six_areas`
        const finalRoundKey = normalizedStage === 'final' ? 'final_round_six_areas' : sixAreaKey
        const sixAreas =
          feedback?.[sixAreaKey] ||
          feedback?.full_rubric?.[sixAreaKey] ||
          feedback?.[finalRoundKey] ||
          feedback?.full_rubric?.[finalRoundKey] ||
