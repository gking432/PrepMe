...
export default function ProcessSpinePage() {
    ...
    const load = async () => {
      ...
      ;(sessions || []).forEach((row: any) => {
        ...
        if (isNewer) {
          ...
          const sixAreas = rubric?.hr_screen_six_areas || rubric?.hiring_manager_six_areas || rubric?.culture_fit_six_areas || rubric?.final_six_areas || rubric?.final_round_six_areas
  ...
  useEffect(() => {
    ...
    Promise.all(
      STAGE_ORDER.map(async (stage) => {
        const sid = process.stages[stage].sessionId
        if (!sid) return [stage, 0] as const
        const params = new URLSearchParams({ stage, sessionId: sid })
        try {
          const res = await fetch(`/api/profile/practice-memory?${params.toString()}`, { cache: 'no-store' })
          const data = res.ok ? await res.json() : null
          const drafts = data?.drafts && typeof data.drafts === 'object' ? data.drafts : {}
          return [stage, Object.keys(drafts).length] as const
        } catch {
          return [stage, 0] as const
        }
      })
    ).then((entries) => {
      setPracticeCompletion(Object.fromEntries(entries))
    })
  }, [process])
  ...
  function primaryPracticeTarget(stage: StageKey) {
    const weaknessCount = process?.stages[stage].weaknesses.length || 0
    return Math.min(2, Math.max(1, weaknessCount))
  }

  function isPracticeComplete(stage: StageKey) {
    return (practiceCompletion[stage] || 0) >= primaryPracticeTarget(stage)
  }
  ...
  function continueToNextStage(stage: StageKey) {
    const next = nextStageAfter(stage)
    if (!next) return
    if (isInterviewStageLocked(stageAccess?.[next]?.hasAccess, next)) {
  ...
  function getSmartCta(stage: StageKey): { label: string; action: () => void; style: 'primary' | 'accent' } | null {
    ...
    if (!isPracticeComplete(stage)) {
      return { label: 'Start practicing', action: () => openPractice(stage), style: 'accent' }
    }
    if (st.attempts <= 1) {
      return { label: stage === 'hr_screen' ? 'Retake HR screen' : 'Retake interview', action: () => launchStage(stage), style: 'primary' }
    }
    const next = nextStageAfter(stage)
    if (next) {
      return { label: `Continue to ${STAGE_META[next].shortName}`, action: () => continueToNextStage(stage), style: 'primary' }
    }
    return null
  }
        ...
        <div className="min-w-0 space-y-5">
          ...
          {activeStatus === 'complete' && (() => {
            ...
            const flowSteps = [
              ...
              { label: 'Practice', done: isPracticeComplete(activeStage), action: () => openPractice(activeStage) },
