...
interface PracticePathProps {
  ...
  transcript?: any
  stageName?: string
  stageKey?: 'hr_screen' | 'hiring_manager' | 'culture_fit' | 'final'
  onClose?: () => void
}
...
export default function PracticePath({ sessionId, weakSignals, transcript, stageName = 'HR Screen', stageKey = 'hr_screen', onClose }: PracticePathProps) {
  ...
  const loadDrafts = useCallback(async () => {
    try {
      const params = new URLSearchParams({ stage: stageKey, sessionId })
  ...
  const primaryTarget = Math.min(2, Math.max(1, nodes.length))
  const completedCount = nodesWithDrafts.filter((n) => n.draft).length
  const primaryCompletedCount = nodesWithDrafts.slice(0, primaryTarget).filter((n) => n.draft).length
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-gradient-to-b from-slate-50 to-white overflow-hidden">
      ...
      <div className="shrink-0 flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 sm:px-6">
        ...
        <div className="text-right">
          <p className="text-xs font-bold text-slate-500">{primaryCompletedCount}/{primaryTarget}</p>
          <p className="text-[10px] font-black uppercase tracking-[0.14em] text-emerald-600">Core</p>
        </div>
      </div>
      ...
      <div className="shrink-0 px-4 pt-3 sm:px-6">
        <div className="h-2 w-full overflow-hidden rounded-full bg-slate-200">
          <div
            ...
            style={{ width: primaryTarget ? `${(primaryCompletedCount / primaryTarget) * 100}%` : '0%' }}
          />
        </div>
      </div>
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`rounded-full px-2 py-0.5 text-[10px] font-black uppercase ${
                            isPrimary ? 'bg-violet-100 text-violet-700' : 'bg-slate-100 text-slate-500'
                          }`}>
                            {isPrimary ? 'core' : 'optional'}
                          </span>
                          {completed && avg !== null && (
          ...
          {totalCount > 0 && (
            <div className="mt-8 rounded-2xl border border-slate-200 bg-slate-50/80 px-4 py-3 text-center">
              <div className="flex items-center justify-center gap-1.5">
                ...
                <p className="text-xs font-bold text-slate-700">Core repairs first ({completedCount}/{totalCount} total done)</p>
              </div>
              <p className="mt-1 text-[11px] leading-5 text-slate-500">
                Complete the core workshops first. Optional repairs are here if you want to keep tightening. Your answers are saved here.
              </p>
            </div>
          )}
        </div>
